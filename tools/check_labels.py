...
def check_label_file(label_path, output_dir, show_plot=True):
    ...
    if show_plot:
        ...
        plt.tight_layout()
        save_filename = f"{split_name}_{folder_name}.png"
...
if __name__ == "__main__":
    ...
    # 深度递归搜索
    search_pattern = os.path.join(PROCESSED_DIR, "**", "labels.npy")
    all_label_files = glob.glob(search_pattern, recursive=True)

    if not all_label_files:
        print(f"警告: 在 {PROCESSED_DIR} 及其子目录下没有找到任何 labels.npy 文件！")
    else:
        ...
        for idx, label_file in enumerate(all_label_files):
            ...
            check_label_file(label_file, output_dir=RESULT_DIR, show_plot=True)
